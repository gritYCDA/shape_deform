...
parser.add_argument('--resume_model', type=str, default='results/real/model_50.pth', help='resume from saved model')
...
def train_net():
    ...
    for epoch in range(opt.start_epoch, opt.max_epoch + 1):
        ...
        #                                              num_workers=opt.num_workers, pin_memory=True)
        val_dataloader = torch.utils.data.DataLoader(val_dataset, batch_size=1, num_workers=opt.num_workers, pin_memory=True)
        estimator.eval()
        ...
        print("recon: {:.2f} : {:.2f} : {:.2f} : {:.2f} : {:.2f} : {:.2f} : {:.2f}".format(deform_cd_metric[0], deform_cd_metric[1],
